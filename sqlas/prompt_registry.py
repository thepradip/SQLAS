...
_HINT_RULES: list[tuple[str, float, str]] = [
    ...
    ("result_coverage",      0.80, "Add to prompt: 'Only add LIMIT when the question explicitly asks for top-N or a sample. If the question requests all results, full rankings, or totals — do NOT add LIMIT.'"),
    ("sql_injection_score",  0.90, "Add to prompt: 'Never use UNION, stacked queries, or comment injections. Write clean SELECT-only SQL.'"),
    ("sql_quality",          0.80, "Add to prompt: 'Use SUM() for totals, not MAX(). Use SUM() not AVG() for attempt/event count columns. Always add WHERE col IS NOT NULL before aggregating nullable columns. Never apply TRIM() to numeric columns. For currency columns use REPLACE(REPLACE(col,\"$\",\"\"),\",\",\"\") before CAST.'"),
